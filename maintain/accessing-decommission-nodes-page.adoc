...
Se um nó de arquivo ainda estiver conetado, você não poderá removê-lo.
 a| 
Conclua as etapas em link:../maintain/considerations-for-decommissioning-admin-or-gateway-nodes.html#considerations-for-archive-node["Considerações para nó Arquivo"] e link:decommissioning-disconnected-grid-nodes.html["desativar o nó desligado"]em seguida .
