...
NOTE: Alguns destinos só estão disponíveis se tiver configurado um servidor syslog externo.
...
Somente nós locais (padrão)
 a| 
As mensagens de auditoria, os logs de eventos de segurança e os logs de aplicativos não são enviados para os nós de administração. Em vez disso, eles são salvos apenas nos nós que os geraram ("o nó local"). As informações de auditoria geradas em cada nó local são armazenadas `/var/local/log/localaudit.log`
...
Nós de administração/nós locais
 a| 
As mensagens de auditoria são enviadas para o log de auditoria (`/var/local/log/audit.log`) em nós de administração e os logs de eventos de segurança e de aplicativos são armazenados nos nós que as geraram.
...
Servidor syslog externo
 a| 
As informações de auditoria são enviadas para um servidor syslog externo e salvas nos nós locais. O tipo de informação enviada depende de como você configurou o servidor syslog externo. Esta opção só é ativada depois de ter configurado um servidor syslog externo.
...
Nó de administração e servidor syslog externo
 a| 
As mensagens de auditoria são enviadas para o log de auditoria (`/var/local/log/audit.log`) em nós de administração e as informações de auditoria são enviadas para o servidor syslog externo e salvas no nó local. O tipo de informação enviada depende de como você configurou o servidor syslog externo. Esta opção só é ativada depois de ter configurado um servidor syslog externo.
