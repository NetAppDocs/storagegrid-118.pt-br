...
NOTE: A senha de instalação temporária é usada apenas durante a instalação. Depois que um nó for adicionado à grade, você pode acessá-lo usando o link:../admin/change-node-console-password.html["senha do console do nó"], que está listado no  `Passwords.txt` arquivo no Pacote de Recuperação.
